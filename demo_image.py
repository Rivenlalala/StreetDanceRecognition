...
def process(input_image, params, model_params):
    ...
    for i in range(18):
        for j in range(len(all_peaks[i])):
            cv2.circle(canvas, all_peaks[i][j][
                       0:2], 4, colors[i], thickness=-1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=str,
                        default='result.png', help='output image')
    parser.add_argument(
        '--model', type=str,
        default='model/keras/model.h5', help='path to the weights file')

    args = parser.parse_args()
    '''
    videoCapture = cv2.VideoCapture()
    videoCapture.open('data/0/0001_30_10.mp4')
